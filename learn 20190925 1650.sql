...
-- Script date 25.09.2019 16:50:56
...
--
CREATE PROCEDURE search(IN countryId INT, IN programId INT, IN languageId INT)
  SQL SECURITY INVOKER
BEGIN
/* insert into log (msg) values (concat(countryId, ', ', programId, ', ', languageId)); */
SELECT
  u.id,
  u.name_en 'name',
  u.found,
  ...
  u.url_pic,
  (SELECT
      GROUP_CONCAT(ll.name_ru SEPARATOR ', ')
    FROM university_languages ul
      JOIN language ll
        ON ll.id = ul.language_id
    WHERE ul.university_id = u.id
    GROUP BY ul.university_id) 'languages',
  (SELECT
      GROUP_CONCAT(pp.name_ru SEPARATOR ', ')
    FROM university_programs up1
      JOIN program pp
        ON pp.id = up1.program_id
    WHERE up1.university_id = u.id
    GROUP BY up1.university_id) 'programs'
FROM university u
  JOIN location lc
    ON lc.id = u.location_id
  JOIN university_type ut
    ON ut.id = u.type_id
...
(22, 372, NULL, '2019-09-13 10:13:41', '2019-09-13 10:13:41'),
...
(5, NULL, 'Braemar College                                                  ', NULL, 1, '2019-09-13 09:43:31', '2019-09-25 12:03:17', NULL, NULL, NULL, NULL, NULL),
